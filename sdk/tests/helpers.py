...
def wm(positions: Sequence[Tuple[float, float]],
       self_id: int = 0,
       stale: Iterable[int] = (),
    ...
    positions[i] is element i's (x, y); `stale` / `inactive` name the IDs
    whose entries carry those flags, and `achieved` supplies the gossiped
    per-element achievement bit the scope="all" predicate reads.
    """
...
def solo(x: float = 0.0, y: float = 0.0, element_id: int = 0) -> List[dict]:
    """A single-element world model — the element sees only itself."""
    return [{"id": element_id, "x": x, "y": y, "is_active": True,
...
TELEMETRY_HEADER = [
    "tick", "wall_time_s", "element_id", "pos_x", "pos_y",
...
def telemetry_row(tick: int, wall_time_s: float, element_id: int,
                  pos: Tuple[float, float], entries: List[dict],
    ...
    same seven keys append_entry_json() records, 'achieved' included —
...
def record_reference_run(script_path: Path, element_id: int = 0,
    ...
    positions = {i: (radius * math.cos(2 * math.pi * i / n_elements),
                     radius * math.sin(2 * math.pi * i / n_elements))
                 for i in ids} if n_elements > 1 else {0: (0.0, 0.0)}
    ...
    for tick in range(max_ticks):
        ...
        for i, ch in choreos.items():
            ...
            if d.type == BSEDirectiveType.MOVE_TO_POINT:
                dx, dy = d.target[0] - x, d.target[1] - y
