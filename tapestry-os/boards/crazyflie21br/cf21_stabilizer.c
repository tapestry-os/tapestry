/*
 * cf21_stabilizer.c — Cascaded attitude controller for the Crazyflie 2.1 brushless
 *
 * See cf21_stabilizer.h for architecture, enabling instructions, and setpoint
 * conventions.
 *
 * PID gain derivation
 * -------------------
...
#include "cf21_imu.h"
#include "crazyflie21br.h"      /* cf21_set_motors() */
#include "crazyflie21br_mix.h"  /* cf21_mix(), cf21_motors_t */

#include <zephyr/kernel.h>
#include <zephyr/logging/log.h>

LOG_MODULE_REGISTER(cf21_stabilizer, LOG_LEVEL_INF);

/* ── Configuration ─────────────────────────────────────────────────────────── */

#define CF21_STAB_STACK_SIZE    2048
#define CF21_STAB_PRIO          K_PRIO_PREEMPT(0)
...
#define CF21_LOOP_DT            (1.0f / 1000.0f)

/* Physical rate and angle limits */
#define CF21_MAX_RATE_RPS       3.49f   /* ±200 deg/s */
#define CF21_MAX_YAW_RATE_RPS   1.75f   /* ±100 deg/s */
...
#define CF21_RP_ILIM    0.3f    /* max ki·integral contribution, output units */
#define CF21_RP_OLIM    0.5f    /* output clamp, [-0.5, +0.5] */

/* Rate PID gains — yaw */
#define CF21_YAW_KP     0.21f
#define CF21_YAW_KI     0.029f
#define CF21_YAW_KD     0.0f
...
static void pid_init(cf21_pid_t *p, float kp, float ki, float kd,
 ...
 */
static float pid_update(cf21_pid_t *p, float error, float dt)
{
...
static void stabilizer_fn(void *a, void *b, void *c)
{
    ...
    cf21_imu_read(&sample);

    while (true) {
        if (cf21_imu_read(&sample) != 0) {
            continue;
        }
        ...
        float yaw_rate_sp = sp.angular.z * CF21_MAX_YAW_RATE_RPS;

#ifdef CONFIG_CF21_ANGLE_MODE
        /* ── Outer angle loop ─────────────────────────────────────────────── */
        cf21_imu_attitude_t att;
        cf21_imu_filter_update(&sample, CF21_LOOP_DT, &att);
        ...
        pitch_rate_sp = sp.angular.y * CF21_MAX_RATE_RPS;
#endif
...
{
    int ret = cf21_imu_init();
    if (ret) {
        LOG_ERR("cf21_imu_init failed: %d", ret);
        return ret;
    }
    ...
    pid_init(&g_pid_pitch, CF21_RP_KP,  CF21_RP_KI,  CF21_RP_KD,
             CF21_RP_ILIM,  CF21_RP_OLIM);
    ...
    k_thread_create(&g_stab_thread, g_stab_stack,
                    ...
                    CF21_STAB_PRIO, 0, K_NO_WAIT);
    k_thread_name_set(&g_stab_thread, "cf21_stab");

    LOG_INF("CF21 stabilizer started (%s mode)",
            IS_ENABLED(CONFIG_CF21_ANGLE_MODE) ? "angle" : "rate");
    return 0;
}
